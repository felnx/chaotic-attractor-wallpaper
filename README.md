...
line-art on a clean background, with the parameter set saved as a short
...
Every generated parameter set is validated with a real orbit + the
largest Lyapunov exponent before it is accepted — diverging, periodic,
and point attractors are thrown away and re-drawn. Chaos is rare (roughly
...
A color pair that's very easy on the eyes (used for the examples above):
brown on beige, `--fg #553E0B --bg #9E8960`.
...
1. **quad2d** integrates the orbit (default 20M steps) and accumulates a
   per-cell pass count in a 32-bit float density grid over the attractor's
   bounding box (expanded by 10% margin). Raw occupancy only — no tone
   curve. It also reports stats (kept/diverged/out-of-box counts, density
   percentiles) and the orbit's *true* extent, so the driver can re-run
   with a grown box if the validation orbit undershot the 20M-step orbit.
...
from, or inspired by, the following — credit where it's due:
